...
let hangman = document.getElementById('canvas')
let hangmanContainer = document.getElementsByTagName('img')[0];
let guess=0;
...
for (let i = 0; i < alphabet.length; i++) {
    ...
    vessel.addEventListener('click', function () {
        ...
        if (!randomWord.includes(alphabet[i])) {
            ...
            attempts.appendChild(attemptsContainer)
            guess++;
            hangmanContainer.src = `hangman${guess}design.png`
            hangman.appendChild(hangmanContainer)
            if (tries < 1) {
